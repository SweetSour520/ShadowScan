...
示例：
    python ShadowScan.py -s 200,301 -x 404\033[0m"""
    print(help_text)
...
def req(url, directory='directory/domain_list.txt', status_include=None, status_exclude=None, threads=10):
    """
    发起请求
    """
    try:
    ...
    with ThreadPoolExecutor(max_workers=threads) as executor:
        futures = {executor.submit(scan_single, t, url, status_include, status_exclude): t for t in targets}
...
def scan_single(target, url, status_include=None, status_exclude=None):
    """
    扫描单个目标
    """
    if exit_flag:
    ...
    try:
        ...
        with print_lock:
            if status_code == 200:
                print(f'\033[92m[+]{target}[{res.status_code}][{title}][{len(res.text)}]\033[0m')
                with open(f'URL/{url}.txt', 'a', encoding='utf-8') as f:
                    f.write(f'[+]\t{target}\t[{res.status_code}]\t[{title}]\t[{len(res.text)}]\n')
            elif status_code == 404:
                print(f'\033[91m[-]{target}[{res.status_code}][{title}][{len(res.text)}]\033[0m')
                with open(f'URL/{url}.txt', 'a', encoding='utf-8') as f:
                    f.write(f'[-]\t{target}\t[{res.status_code}]\t[{title}]\t[{len(res.text)}]\n')
            else:
                print(f'\033[93m[*]{target}[{res.status_code}][{title}][{len(res.text)}]\033[0m')
                with open(f'URL/{url}.txt', 'a', encoding='utf-8') as f:
                    f.write(f'[*]\t{target}\t[{res.status_code}]\t[{title}]\t[{len(res.text)}]\n')
...
def main():
    ...
    thread_input = input('\033[96m请输入线程数：\033[0m')
    ...
    if directory_choice == '1' or directory_choice == '':
        ...
        req(r_domain, status_include=args['status_include'], status_exclude=args['status_exclude'], threads=threads)
    elif directory_choice == '2':
        directory = input('请输入字典路径：')
        print('\033[96m—————————————————— 开始扫描 ——————————————————\033[0m')
        req(r_domain, directory, status_include=args['status_include'], status_exclude=args['status_exclude'], threads=threads)
    else:
